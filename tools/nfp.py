...
from tools.geofunc_rh import GeoFuncRH
...
class RHNFP(object):
    def __init__(self,poly1,poly2,**kw):
        self.stationary=copy.deepcopy(poly1) # 坐标点集
        self.sliding=copy.deepcopy(poly2) # 坐标点集
        start_point_index=GeoFuncRH.checkBottom(self.stationary) # 获得最左下角的点的序号
        self.start_point=[poly1[start_point_index][0],poly1[start_point_index][1]] # 在这儿
        self.locus_index=GeoFunc.checkTop(self.sliding) # 轨迹点的序号
        # 如果不加list则original_top是指针
        self.original_top=list(self.sliding[self.locus_index]) # 记录原始的top点的位置信息
        GeoFunc.slideToPoint(self.sliding,self.sliding[self.locus_index],self.start_point) # 这里对sliding进行了平移
        self.start=True # 判断是否初始
        self.nfp=[]
        self.rectangle=False
        if 'rectangle' in kw:
    ...
    def main(self):
        i=0
        if self.rectangle: # 若矩形则直接快速运算 点的index为左下角开始逆时针旋转 # 注意是两个都为矩形的情况
            width=self.sliding[1][0]-self.sliding[0][0]
            height=self.sliding[3][1]-self.sliding[0][1]
            self.nfp.append([self.stationary[0][0],self.stationary[0][1]])
            self.nfp.append([self.stationary[1][0]+width,self.stationary[1][1]])
            ...
            # while i<7:
                # print("########第",i,"轮##########")
                touching_edges=self.detectTouching() # 返回相交的两条边
                all_vectors=self.potentialVector(touching_edges)
                if len(all_vectors)==0:
                    self.error=-2 # 没有可行向量
                    raise Exception("Error:没有可行向量")
                    break
    ...
    def detectTouching(self):
        touch_edges=[]
        stationary_edges,sliding_edges=self.getAllEdges() # 由点依次链接得到所有的边
        # 不动的poly的边与所有的slide边都进行相交计算
        for edge1 in stationary_edges:
            for edge2 in sliding_edges:
                inter=GeoFunc.intersection(edge1,edge2) # 返回相交点
                if inter!=[]:
                    pt=[inter[0],inter[1]] # 交叉点
                    edge1_bound=(GeoFunc.almostEqual(edge1[0],pt) or GeoFunc.almostEqual(edge1[1],pt)) # 是否为边界点
                    edge2_bound=(GeoFunc.almostEqual(edge2[0],pt) or GeoFunc.almostEqual(edge2[1],pt)) # 是否为边界点
                    stationary_start=GeoFunc.almostEqual(edge1[0],pt) # 是否开始
                    orbiting_start=GeoFunc.almostEqual(edge2[0],pt) # 是否开始
                    touch_edges.append({
                        "edge1":edge1,
                        "edge2":edge2,
                        "vector1":self.edgeToVector(edge1), # 得到边的向量
                        "vector2":self.edgeToVector(edge2), # 得到边的向量
                        "edge1_bound":edge1_bound,
                        "edge2_bound":edge2_bound,
                        "stationary_start":stationary_start,
                        "orbiting_start":orbiting_start,
    ...
    # 获得两个多边形全部边
    def getAllEdges(self):
        return GeoFuncRH.getPolyEdges(self.stationary),GeoFuncRH.getPolyEdges(self.sliding)
    
    # 判断是否结束
    def judgeEnd(self):
        '''根据当前的locus_pt与当前其起始点的位置关系来判断是否结束'''
        sliding_locus=self.sliding[self.locus_index]
        main_bt=self.start_point
        if abs(sliding_locus[0]-main_bt[0])<0.1 and abs(sliding_locus[1]-main_bt[1])<0.1:
            if self.start==True:
